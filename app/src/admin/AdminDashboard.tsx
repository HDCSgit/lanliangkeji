...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [banners, products, news, partners] = await Promise.all([
        DataStore.getBanners(),
        DataStore.getProducts(),
        DataStore.getNews(),
        DataStore.getPartners(),
      ]);
      ...
      setStats({
        ...
        news: news.filter((n) => n.isActive).length,
        partners: partners.filter((p) => p.isActive).length,
      });
      ...
      setPopularProducts(products.filter((p) => p.isActive).slice(0, 4));
    };
