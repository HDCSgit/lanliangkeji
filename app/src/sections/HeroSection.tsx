...
import { DataStore, defaultStats } from '@/data/store';
...
const HeroSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadBanners = async () => {
      const loaded = await DataStore.getBanners();
      setBanners(loaded.filter((b) => b.isActive).sort((a, b) => a.order - b.order));
    };
    loadBanners();
  }, []);
  ...
  useEffect(() => {
    const loadStats = async () => {
      const loaded = await DataStore.getStats();
      setStats(loaded);
    };
    loadStats();
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-32 lg:pt-24 pb-16">
