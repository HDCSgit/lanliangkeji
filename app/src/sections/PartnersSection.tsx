...
import { DataStore, defaultPartners } from '@/data/store';
...
const PartnersSection: React.FC = () => {
  ...
  const [partners, setPartners] = useState<Partner[]>(defaultPartners.filter((p) => p.isActive).sort((a, b) => a.order - b.order));

  useEffect(() => {
    const loadPartners = async () => {
      const loaded = await DataStore.getPartners();
      setPartners(loaded.filter((p) => p.isActive).sort((a, b) => a.order - b.order));
    };
    loadPartners();
  }, []);
