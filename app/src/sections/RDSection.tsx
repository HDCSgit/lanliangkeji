...
import { DataStore, defaultRDInfo, defaultStats } from '@/data/store';
import type { RDInfo, Stat } from '@/types';
...
const RDSection: React.FC = () => {
  ...
  const [rdInfo, setRdInfo] = useState<RDInfo>(defaultRDInfo);
  const [stats, setStats] = useState<Stat[]>(defaultStats);

  useEffect(() => {
    const loadData = async () => {
      const [loadedRd, loadedStats] = await Promise.all([
        DataStore.getRDInfo(),
        DataStore.getStats(),
      ]);
      setRdInfo(loadedRd);
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <div className="relative rounded-2xl overflow-hidden">
              <img
                src="/images/rd/rd-equipment.jpg"
                alt="实验室设备"
