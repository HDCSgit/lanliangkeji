...
import { DataStore, defaultServices } from '@/data/store';
...
const ServicesSection: React.FC = () => {
  ...
  const [services, setServices] = useState<Service[]>(defaultServices.filter((s) => s.isActive).sort((a, b) => a.order - b.order));

  useEffect(() => {
    const loadServices = async () => {
      const loaded = await DataStore.getServices();
      setServices(loaded.filter((s) => s.isActive).sort((a, b) => a.order - b.order));
    };
    loadServices();
  }, []);
          ...
          {services.map((service) => {
            ...
            return (
              ...
              >
                ...
                {/* Content */}
                <h3 className="relative text-xl font-bold text-ocean-deep mb-3">
                  {service.name}
                </h3>
                <p className="relative text-gray-600 text-sm mb-4 line-clamp-3">
                  {service.description}
                </p>
                ...
                <div className="relative flex flex-wrap gap-2 mb-4">
                  {service.features?.slice(0, 2).map((feature, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 bg-ocean-blue/10 text-ocean-blue text-xs rounded-full"
                    >
                      {feature}
                    </span>
                  ))}
                ...
                <Link
                  to="/products"
                  className="relative inline-flex items-center gap-2 text-ocean-blue font-medium text-sm hover:gap-3 transition-all"
                >
                  了解详情
